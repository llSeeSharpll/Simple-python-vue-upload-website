    ...
    mutations: {
        setLogged(state, loginResult) {
            ...
            if (loginResult.success) {
                ...
                sessionStorage.username=loginResult.username
            } else {
        ...
        authenticateUserAndSetToken({ commit, dispatch }, payload) {
            return new Promise(function (resolve, reject) {
                ...
                axiosInstance.post('token', payload)
                    .then(function (response) {
                        commit('setIsProcessing', false);
                        commit('setLogged', { success: true,username:payload.username, token: response.data.token, message: "Credentials accepted!" });
