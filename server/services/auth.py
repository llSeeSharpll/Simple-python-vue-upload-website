...
import sys
sys.path.append('../')
from entities.user import App_User,Image_User
...
class Auth():
    ...
    def validateRegisterData(self, username, password) -> LoginTokenResult:
        ...
        result = self.dbSession.query(
            App_User).filter_by(username=username).first()
    ...
    def load_user(self, user_id):
        return self.dbSession.query(App_User).get(int(user_id))

    def GetUserByEmail(self, username):
        return self.dbSession.query(App_User).filter_by(username=username).first()
    ...
    def GetUserByToken(self, jwt):
        ...
        if activeSession is not None:
            return self.dbSession.query(App_User).get(activeSession.userId)
        else:
            return None
    ...
    def get_image(self):
        return self.dbSession.query(Image_User.image,App_User.username).filter_by(userId=App_User.userId).all()